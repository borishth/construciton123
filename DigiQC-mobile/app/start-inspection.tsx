...
export default function StartInspectionScreen() {
  ...
  const handleStart = () => {
    if (!projectName.trim() || !siteName.trim() || !checklistType || !inspectorName.trim()) {
      alert('Please fill in all required fields (Project, Site, Checklist Type, and Inspector).');
      return;
    }
    router.push({
      pathname: '/checklists' as any,
      params: { projectName, siteName, structureType, checklistType, date, inspectorName },
    });
  };
        ...
        <View style={{ flexDirection: 'row', gap: 12 }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <TouchableOpacity style={styles.inputBox} onPress={() => setShowTypeList(!showTypeList)} activeOpacity={0.8}>
              ...
              <MaterialIcons name={showTypeList ? 'expand-less' : 'expand-more'} size={18} color="#727785" />
            </TouchableOpacity>
          </View>
        </View>
